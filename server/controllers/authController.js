...
const signUpController = async (req, res) => {
  try {
    const { name, email, userRole, phoneNumber, password } = req.body;

    if (!name || !email || !password || !userRole || !phoneNumber) {
...
const loginController = async (req, res) => {
  try {
    ...
    if (user.role !== role) {
      return res.send(error(404, "User with this role not found"));
    }
